    ...
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted mt-1">Active job roles and their interview pipeline status.</p>
        </div>
        <button className="flex items-center gap-2 rounded-lg bg-accent text-white px-4 py-2 text-sm font-medium hover:bg-accent/90 transition-colors">
      ...
      {roles.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          {roles.map(role => (
            <div key={role.name} className={`relative rounded-xl border bg-card p-6 shadow-sm transition-all hover:shadow-md ${role.pending > 0 ? 'border-accent/40 ring-1 ring-accent/20' : 'border-border'}`}>
              {role.pending > 0 && (
              ...
              <div className="flex items-start gap-3 mb-4">
                <div className="h-10 w-10 rounded-lg bg-accent-bg flex items-center justify-center shrink-0">
